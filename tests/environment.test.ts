...
describe('canonical scene registry', () => {
  it('defines six complete, distinct scene identities', () => {
    expect(VENUE_IDS).toHaveLength(6);
    expect(Object.keys(SCENE_DEFINITIONS)).toEqual(VENUE_IDS);
    expect(new Set(VENUE_IDS.map((id) => SCENE_DEFINITIONS[id].label)).size).toBe(6);
  ...
  it('keeps outdoor and indoor lighting families explicit', () => {
    expect(VENUE_IDS.filter(isOutdoorVenue)).toEqual(['outdoor-club', 'clay-terrace', 'grass-park-night']);

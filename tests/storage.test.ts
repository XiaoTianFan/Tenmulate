...
describe('local application data', () => {
  ...
  it('migrates optional ball focus off and persists its bounded maximum blur', () => {
    ...
    expect(loadAppData().preferences.ballFocus).toEqual({enabled:false,maxBlurPx:3});
    saveAppData({...DEFAULT_APP_DATA, preferences:{...DEFAULT_PREFERENCES,ballFocus:{enabled:true,maxBlurPx:4.5}}});
    expect(loadAppData().preferences.ballFocus).toEqual({enabled:true,maxBlurPx:4.5});
    ...
    expect(loadAppData().preferences.ballFocus).toEqual({enabled:true,maxBlurPx:6});
  });

...
const document = await io.read(fileURLToPath(new URL(`${venue}.raw.glb`, build)));
...
const reference = await readFile(new URL(`${venue}.raw.glb`, build));
...
const filename = `${venue}.${sha256.slice(0, 12)}.glb`;
...
const manifest = {
  ...
  url: `/assets/venues/${venue}/${filename}`, bytes: bytes.length, sha256,

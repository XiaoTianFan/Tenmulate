...
export function SetupScreen({ route, cameraPositionPresets = DEFAULT_CAMERA_POSITION_PRESETS, perspectivePresets = DEFAULT_PERSPECTIVE_PRESETS, initialPreferences, practiceConfigs, onSaveConfig, onRestoreBallFocus, onRoute, onStart, onSaveCameraPositionPreset, onSavePerspectivePreset, onPreferencesChange }: SetupScreenProps) {
  ...
  return (
    <main className="app-shell setup-shell">
      ...
      <section className="practice-layout">
        ...
        <section className="preview-column" aria-label="Live court preview">
          <div className="setup-court-view" ref={overviewContainer} data-camera-eye-height={eyeHeight.toFixed(3)}>
            ...
            <div className="editor-view-tools"><button type="button" aria-pressed={overview} onClick={() => setOverview(value => !value)}>{overview ? 'Back to player view' : 'Top-down court'}</button>{overview ? <span><i className="return-swatch"/>Your return <i className="landing-swatch"/>Opponent landing</span> : null}</div>
          ...
          <div className="preset-toolbar">
            <div className="preset-group" aria-label="Camera position presets">
              ...
              <div className="preset-row"><div className="preset-options">{cameraPositionPresets.map((preset) => <button key={preset.id} type="button" className={selectedPositionPreset === preset.id ? 'preset-chip active' : 'preset-chip'} onClick={() => applyCameraPosition(preset)} onContextMenu={(event) => { event.preventDefault(); updatePositionPreset(preset); }}>{preset.name}</button>)}</div><button className="preset-add" type="button" aria-label="Save camera position preset" title="Save the current camera location and eye height" onClick={() => { setPresetName('My position'); setDialog('new-position'); }}><Save size={14} /><span>Save preset</span></button></div>
            </div>
            <div className="preset-group" aria-label="Perspective presets">
              <header><span><Eye size={14} /> Perspectives</span><small>Right-click to save preset</small></header>
              <div className="preset-row"><div className="preset-options">{perspectivePresets.map((preset) => <button key={preset.id} type="button" className={selectedPerspectivePreset === preset.id ? 'preset-chip active' : 'preset-chip'} onClick={() => applyPerspective(preset)} onContextMenu={(event) => { event.preventDefault(); updatePerspectivePreset(preset); }}>{preset.name}</button>)}</div><button className="preset-add" type="button" aria-label="Save perspective preset" title="Save the current viewing direction and field of view" onClick={() => { setPresetName('My perspective'); setDialog('new-perspective'); }}><Save size={14} /><span>Save preset</span></button></div>
            </div>
        ...
        <aside className="inspector" aria-label="Practice configuration">
          ...
          <SetupSection title="Opponent shot" subtitle="Shot type and spin" open>
            ...
            {shotType!=='serve'?<label className="select-field"><span>Stroke side</span><select aria-label="Stroke side" value={practiceStroke} onChange={event=>setPracticeStroke(event.target.value as 'forehand'|'backhand'|'auto')}><option value="auto">Automatic</option><option value="forehand">Forehand</option><option value="backhand">Backhand</option></select></label>:null}
          </SetupSection>
          <SetupSection title="Ball & rhythm" subtitle="Flight, speed, timing" open>
            <label className="toggle-field"><span>Trajectory</span><button type="button" role="switch" aria-label="Trajectory" aria-checked={trajectoryEnabled || overview} disabled={overview} title={overview ? 'Always shown in top-down view' : undefined} className={trajectoryEnabled || overview ? 'toggle active' : 'toggle'} onClick={() => setTrajectoryEnabled((value) => !value)}><span /></button><small>{trajectoryEnabled || overview ? 'On' : 'Off'}</small></label>
          ...
          <SetupSection title="Your return" subtitle="Blue landing zone" open><button type="button" className="text-action" onClick={() => setOverview(true)}>Edit return landing zone on court</button></SetupSection>
          ...
          <div className="inspector-actions"><button className="secondary-button" type="button" disabled={saving} onClick={() => void performSave(() => onSaveConfig(pendingPreferences.current))}><Save size={16}/>{saving ? 'Saving…' : 'Save config'}</button><button className="primary-button" type="button" disabled={launching || preview.pending || !!preview.error || !!previewSession.planningIssues?.length} onClick={requestStart}><Play size={16}/>{launching ? 'Preparing practice…' : 'Start practice'}</button></div>
        </aside>
      </section>
      ...
      {dialog === 'new-position' || dialog === 'new-perspective' ? <Modal title={dialog === 'new-position' ? 'New camera position' : 'New perspective'} onClose={() => setDialog(null)} actions={<><button className="secondary-button" type="button" onClick={() => setDialog(null)}>Cancel</button><button className="primary-button inline" type="button" disabled={saving} onClick={createPreset}>Save preset</button></>}><p>{dialog === 'new-position' ? 'Reuse this camera location and eye height in another practice setup.' : 'Reuse this viewing direction and field of view from any camera position.'}</p><label className="stack-field"><span>Preset name</span><input autoFocus maxLength={40} value={presetName} onChange={(event) => setPresetName(event.target.value)} /></label></Modal> : null}
    </main>
  );
}

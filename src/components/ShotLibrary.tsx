...
export function ShotLibrary({ savedShots, projectShotIds, notice, writable, onAdd, onCreate, onDelete, onMenuChange }: {
  ...
  return <aside className="event-library">
    ...
    <p className="saved-shot-count" role="status">{notice || 'Click or drag to add. Right-click for options.'}</p>
    <div className="event-list">
      ...
      </button><button type="button" className="shot-library-options" aria-label={`Options for ${item.name}`} aria-haspopup="menu" aria-expanded={menu?.id === item.id}
    ...
    <footer className="shot-library-footer"><button ref={createRef} type="button" className="secondary-button full-width" disabled={!writable} onClick={() => { setType('all'); setSource('all'); onCreate(); }}><Plus size={17}/> Add New Shot</button></footer>
    {menu && selected ? createPortal(<div ref={menuRef} className="shot-library-menu" role="menu" aria-label={`Options for ${selected.name}`}>
      <button type="button" role="menuitem" disabled={busy || !writable && projectShotIds.includes(selected.id)} onClick={() => void remove()}><Trash2 size={16}/>{busy ? 'Deleting…' : 'Delete shot from library'}</button>

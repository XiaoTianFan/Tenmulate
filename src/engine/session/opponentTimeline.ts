...
export type StrokeId = 'forehand' | 'backhand' | 'serve';
...
export const MAX_OPPONENT_SPEED = 3.2;
...
export const strokeForShot = (shot: ShotDefinitionV1, index: number): StrokeId => {
  ...
  if (shot.stroke) return shot.stroke;
  if (shot.backhandStyle) return 'backhand';
  const relativeSide = shot.source.x * (shot.opponentHand === 'left' ? -1 : 1);
  return Math.abs(relativeSide) > 0.4 ? relativeSide < 0 ? 'backhand' : 'forehand' : index % 2 ? 'backhand' : 'forehand';
};
...
export const minimumMotionGap = (previous: MotionRepetition, next: MotionRepetition): number => {
  ...
  const travel = Math.max(.18, 1.5 * distance / MAX_OPPONENT_SPEED);
...
export const sampleOpponentTimeline = (events: readonly MotionEvent[], time: number): MotionSample | null => {
  ...
  const start = previous.end, end = Math.min(next.start, start + Math.max(.4, 1.5 * distance / 2.6)), duration = end - start;
  if (time >= end) {
  ...
  const progress = smoothStep((time - start) / duration), rootAt = (t: number) => lerpPoint(previous.root, next.root, smoothStep((t - start) / duration));
  const root = rootAt(time), yaw = yawLerp(previous.yaw, next.yaw, progress);
  ...
  const movement: MotionId = Math.abs(localX) > Math.abs(localZ) ? localX < 0 ? 'move-right' : 'move-left' : localZ > 0 ? 'move-forward' : 'move-backward';
  const period = Math.min(.65, Math.max(.32, duration / Math.max(1, Math.ceil(distance / .85))));
  const phase = (time - start) / period, blend = smoothStep((time - start) / .14) * smoothStep((end - time) / .14);
  const foot = (side: 'left' | 'right'): Vec3 => {
    const offset = side === 'right' ? 0 : .5;
    const cycle = Math.floor(phase - offset), u = phase - offset - cycle;
    const plantStart = start + (cycle + offset - .5) * period, plantEnd = plantStart + period;
    const localFoot = [side === 'left' ? .26 : -.26, .087, side === 'left' ? .05 : -.025];
    const anchored = (t: number): Vec3 => {
      const base = rootAt(t), plantYaw = yawLerp(previous.yaw, next.yaw, smoothStep((t - start) / duration));
      const local = rotateMotionPoint(localFoot, plantYaw, next.hand);
      return { x: base.x + local.x, y: local.y, z: base.z + local.z };
    };
    const swing = u < .5, a = anchored(plantStart), b = anchored(plantEnd);
    const position = swing ? lerpPoint(a, b, smoothStep(u * 2)) : b;
    const moving = { ...position, y: position.y + (swing ? Math.sin(u * Math.PI * 2) * .11 : 0) };
    const local = rotateMotionPoint(localFoot, yaw, next.hand);

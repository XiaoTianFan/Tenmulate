...
- The rendered zone highlights on hover. Left-drag its interior to move along the court plane, a side to resize one dimension, or a corner to resize both with opposite bounds anchored. Pointer motion previews scene geometry; release commits one edit and cancellation restores it. Left-drag outside it adjusts the camera. Pointer capture keeps these gestures separate; keyboard movement, dimension fields and touch access remain available. The actual-bounce marker hides during an uncommitted edit; there is no center handle or arrow.
- Practice, Editor and drill playback reuse one mounted 3D scene and unchanged assets. Pages without a court retain those resources while suspending rendering.
- Left/right corner camera presets stand 2.8 m behind the baseline and 3.6 m laterally from center. Volley stands on the center line, 1 m toward the net from the service T. All three aim at the opposite baseline center and preserve the selected FOV.
...
[ADR-0018](decisions/0018-uniform-landing-zones-and-continuous-preview.md), the interaction refinements in [ADR-0019](decisions/0019-direct-landing-zone-manipulation.md) and [ADR-0020](decisions/0020-shared-court-and-transactional-zone-editing.md), and the [shared-court receipt](development/shared-court-and-zone-resize-2026-09-08.md) define the implemented contract and its evidence.
...
- A local editor with a filtered default/saved shot library, drag/drop timeline insertion and reordering, right-click/Delete removal and Undo. Ball sliders precede Perspective; WASD/Shift and Page Up/Down move the shot camera.
- Reusable local shot snapshots with opponent hand/stroke, automatic contact positioning, camera position/direction/FOV, incoming and return landing zones, launch speed, spin/rpm, variation, bounce, trajectory fitting, interval and movement/stroke rhythm. Copying or later updating a preset must not mutate existing drill events.
- Per-event return landing zones on the opponent half, independent of camera position. The pseudo-return starts on the resolved incoming flight; its next physical intercept determines the opponent contact and recovery route. A centered top-down view edits both zones directly. Infeasible links remain explicit new feeds. [ADR-0035](decisions/0035-court-space-returns-and-shot-library-editor.md) supersedes the camera-relative return space.
...
| BALL-08 | Configure physical landing interactively. | Left-drag the rendered zone interior to move it, or its sides/corners to resize it on the court. Preview without trajectory/motion recompilation during the gesture, then commit one complete edit on release. |

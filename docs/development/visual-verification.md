...
| --- | --- | --- | --- |
| Blue hard arena | Open roof, blue seating bowl, dark ad ring, tournament-scale enclosure | Four-sided instanced blue bowl, canopy aperture, unbranded ad ring, score display, and aligned canopy fixtures frame the player view | Composition and color hierarchy pass; add upper-deck/concourse articulation during owner polish. |
| Sunset clay bowl | Clay deck, green/tan tiers, asymmetric upper mass, dusk sky | Warm-seat compact bowl, added side deck masses, open canopy, clay procedural deck, continuous 18:30 solar state | Identity pass after correcting an initial post-sunset black frame; sky cloud/terrain silhouette remains a polish target. |
| Grass center court | Striped grass, green bowl, white roof/truss identity, open daylight | Strong grass stripe shader, green four-sided bowl, pale canopy/trusses, dark surround, score display, and open sky aperture | Clear identity and gameplay-readability pass; add richer roof mechanics and aisle/vomitory detail. |
...
| Dynamic atmosphere | Sky must drive light/time and react to weather | Three.js `Sky`, solar sun, hemispheric fill, fog, PMREM environment, clear/overcast/rain and calibrated exposure all update together | Functional/visual pass. The first browser frame exposed over-energy from daylight floodlights; fixed before commit. |
...
| Indoor halls | No seating, aligned lamps, upright semicircle grass roof | All three halls have no bleacher groups; fixture lenses and spotlights share transforms; grass hall shows a clear upright barrel arch | Pass. Far-wall and acoustic/service detail remain visually simple by design. |
...
| Runtime health | Switching and atmosphere updates must not leak or error | Outdoor club, three arenas, rain, and barrel hall switched in the in-app browser; 61 tests and production build pass; zero console errors | Correctness pass. A non-fatal GPU precision warning and reference-device profiling remain open. |

The three owner-supplied photographs were used only for massing, palette, venue hierarchy, and lighting calibration. Identifiable logos, sponsor art, and venue-specific trade dress were deliberately excluded. Final evidence captures are under ignored `output/playwright/` and were inspected directly with `view_image` after the browser loop.

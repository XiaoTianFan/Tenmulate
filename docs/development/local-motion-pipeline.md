...
- **Updated:** 2026-09-06, volley addition after revision 6
...
Current results are recorded in the lab's `docs/volley-motion.md` and `docs/evidence/volleys.json`; the revision-6 ledger remains historical evidence for the preserved corrections. All 15 clips pass with zero glTF errors/warnings and zero joint violations. Volley-specific transform gates reject the old slice clips when relabeled as volleys. The actual controller passes 11 automated mixed events for both hands at 240 Hz after blending/IK, with .079 m minimum groundstroke backhand finish clearance and less than .002 mm maximum contact error. **20 lab JavaScript tests, 4 Python rhythm tests, 208 frontend tests / 21 files and production build** pass. New tests cover volley presets/practice, spin-independent selection, mirrored and arbitrary-seek contact, uniform rates and deliberately short cadence. Browser review covered full volley phase sequences, front/rear/side model views, mirrored gameplay contacts and ordinary playback from approach run through forehand volley, walking recovery and backhand volley. Source previews decoded at the selected contacts. No browser errors or warnings were captured in this pass; the existing build chunk-size warning remains. Desktop visual review remains separate from owner technique approval.
...
Edit `scripts/motion_refinements.py` for the current pose/joint corrections, `scripts/motion_recipes.py` for base poses, `scripts/motion_volleys.py` for the two volleys and `config/motion-rhythm.json` for timing. Inspect the rebuilt GLB before accepting choreography. The editable master is `output/tennis-motion-master.blend`; manual Blender edits must be carried back to reproducible controls.

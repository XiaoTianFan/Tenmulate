...
Revision 3 supersedes the choreography rejected in the owner's MotionLab review. The sibling lab now produces 13 clips through visual reference-led anatomical authoring, including ordinary running and walking. Extracted poses are not production inputs. Both MotionLab and MotionSequence show paired front/rear references below 3D, with missing or secondary performers identified. Forehand, backhand, serve, slices, ready and all steps use calibrated joint chains, rigid grips and constrained recovery.

The 1.65 MB library has zero glTF errors/warnings. All 13 clips pass the 120 Hz anatomical/choreography gates; all 12 old clips fail the new anatomy gate. The real controller passes both hands after blending and foot IK with zero joint violations and approximately 0.0011 mm maximum contact error. **202 tests / 21 files**, 11 lab tests and the production build pass. [Current contract, source limits and evidence](local-motion-pipeline.md). Exact style, missing clean slice/primary rear-serve references, device endurance and owner visual approval remain separate. Historical notes below describe their original stages.

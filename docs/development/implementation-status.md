...
- **Current implementation:** Blender hard/clay opt-in delivery with clay seating revision 2. Initial source commits `f06675a` and `76ebd88` and older stage commits below remain historical.
...
The Philippe-Chatrier-informed clay venue is built in Blender and integrated under the existing optional authored-venue switch, alongside hard. Seating revision 2 has 13,664 ash seats in four court-aligned stands: fixed seat columns, perpendicular section aisles, aligned terrace entrances and rectangular front boxes, with symmetric fans confined to chamfered corners. Green recessed perimeter entrances, original clay PBR textures, a glazed concourse and ten nested fixed-open roof wings remain. Both variants retain regulation gameplay registration, shared runtime lighting and procedural loading/failure/offline fallback. The default remains procedural; the hard asset is unchanged. [ADR-0010](../decisions/0010-blender-clay-arena-and-multi-venue-boundary.md), [build/source details](clay-arena.md) and [Stage 13 visual evidence](visual-verification.md#stage-13-clay-arena-orthogonal-seating--2026-09-05) define the current boundary.

Verification: 144 tests / 15 files and production build pass; actual GLB hash/decode/anchors/seats/clearance/roof checks and new fixed seat-column/aisle-edge checks pass. Revision 2 passed desktop/mobile production review and camera/time interactions. Both cached assets offline, forced clay-load failure and zero-download default path were exercised in the initial integration; unchanged runtime paths were not re-exhausted for this asset-only revision. Target-device frame-time/memory/cold-load profiling, owner visual sign-off, tennis mocap and public deployment remain open; this is local implementation, not release approval.

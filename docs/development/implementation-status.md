...
ownership. Integration release: 697 tests / 69 files and build/motion guard pass.
Browser DSP/lifecycle checks use explicit byte fixtures because host media HTTP
returns empty 204s. Production timing/offline/device and owner listening remain
open; see the [audio ledger](audio-system.md). No merge or deployment.
...
and immediately stops/reset the old preview. Worker reuse, bounded caches and a
smaller initial preview batch reduce repeated work. The shared renderer and player
clock reset before playback without reloading the court. Development/production
browser timing and cold-start limitations: [startup contract](session-startup.md).
...
within the configured zones. Each accepted intercept must support grounded
opponent movement and a legal outgoing ball. Automatic player-shot defaults may
change to keep a connection; configured shot/phase constraints remain explicit.
The captured reload stall now passes 12 continued batches with physical contact
and clock continuity. See [ADR-0054](../decisions/0054-rally-feasibility-before-variation.md)
and the [verification receipt](rally-search-2026-09-13.md), including numerical
search limits and the unrelated catalog-dependent full-suite failures.
...
name/ID replacement and loopback development/preview access. The editor now retains
per-drill work independently of explicit project saves. Direct editing applies to
every library drill; copying remains optional. Clean snapshots refresh from newer
project data, while unfinished work survives navigation and test playback.

All **546 tests / 50 files**, production build and active motion/cache guard pass.
**33 production Edge checks** cover project writes/readback across browser contexts,
overwrites, failed saves, recovery, import/new drills, gameplay return and responsive
layouts. QA uses an isolated temporary catalog. The actual local 5173 preview API
is active; no user browser records or project drill values were changed by QA.
See the [saving guide and receipt](project-drill-saving-2026-09-10.md) and
[ADR-0043](../decisions/0043-project-drill-catalog-and-editor-drafts.md).

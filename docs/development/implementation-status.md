...
- Added a renderer-owned animated fBM cloud dome whose coverage, density, color, opacity, and motion respond to weather, solar elevation, and configured wind. It renders independently of the environment-map sky to keep PMREM updates bounded.
...
- Replaced the arena blockout stands with eight reusable lower/upper stadium sections per arena. Each section uses instanced seat pans/backs, stepped concrete, aisle gaps, front and aisle rails; the bowl adds concourse slabs, dark vomitories, segmented unbranded ad marks, venue-specific deck colors, segmented roof panels, catwalk bands, and repeated braces.
...
- Calm and wind trajectory tests, legacy storage normalization, nine-venue registry, no-seating indoor contract, eight-section multi-tier arena contract, procedural-surface switching, fixture/lens alignment, and upright roof coverage bring the full suite to 61 passing tests.
- `npm run build` passes. The renderer chunk is approximately 164.3 kB gzip and the full generated PWA precache remains approximately 1.82 MiB with no external venue texture/model payload added.
- In-app browser verification exercised the original outdoor club, all three arena identities, clear/golden/rain atmosphere changes, and the corrected barrel-vault hall. The loop caught and fixed cloud depth rejection and an over-bright exposure experiment. Final switching produced zero browser console errors or warnings. The references and final captures were inspected together; curved bowl geometry, detailed roof mechanics, architectural weathering, foreground reference-device profiling, and owner visual approval remain explicit gates.

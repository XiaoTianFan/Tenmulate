...
| Environment format | Typed Three.js fallback compositions plus opt-in Blender-authored GLB | ADR-0005/0006 remove generated worlds and splats; ADR-0009/0010 permit hard and clay authored arenas while gameplay coordinates, atmosphere and lighting stay TypeScript-owned. |
...
ADR-0009 and ADR-0010 add two implemented Blender venues. Source, provenance, setup and reproducible builds are documented in [Blender venues](development/blender-venues.md) and the [clay arena record](development/clay-arena.md). The visible court/net may be authored in the GLB, but regulation coordinates and gameplay authority stay in TypeScript. A failed/unavailable authored venue retains the tested procedural presentation. `AUTHORED_VENUES` registers only hard and clay; each manager validates its requested ID, hash, size and independent gameplay anchors before presentation replacement.
...
- The critical route loads UI, the selected typed Three.js venue composition, court, ball, and drill first. The neutral opponent/animation bundles and the opt-in hard/clay arenas are external lazy GLB assets. An arena's presentation replaces the procedural court/venue only after its hash and gameplay anchors pass; the ordinary default path does not fetch either arena. Deselecting a loading manager aborts it; completed inactive assets remain cached until scene disposal, so two-asset residency requires device profiling.
...
- The authored arenas use exported PBR, CC0 concrete maps, original acrylic/clay/ash textures and baked vertex occlusion. Native hard/clay wetness updates PBR roughness; alternate surfaces borrow the shared procedural court materials without transferring ownership. Meshopt decoding and WebP textures are lazy runtime dependencies; large GLBs use a dedicated hash-keyed runtime cache (four visited URLs in total), never the shell precache. Manifests are network-first with cached offline fallback. Four authored clay light anchors position the fixed court lights independently of the parked roof.
